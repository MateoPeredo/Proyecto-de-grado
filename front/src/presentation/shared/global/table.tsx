...
export const Table = <T extends object>({ data, columns }: PropsTable<T>) => {
  ...
  // Filtrar los datos basados en el valor de búsqueda y la columna seleccionada
  ...
  // Usar los datos filtrados en la tabla
  const table = useReactTable({
    data: filteredData, // Pasar los datos filtrados aquí
    columns,
    state: {
      globalFilter,
    },
    ...
    <section className="w-full h-full rounded-md overflow-hidden shadow-md p-4 bg-white">
      ...
      <section className="border rounded-md">
        <article className="flex justify-between p-4">
          <FilterButton onClick={() => setGlobalFilter("")}>
            Limpiar
          </FilterButton>
